...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Spesifikasi dan Harga ${car.name} Terbaru - Agung Toyota`,
    description: `Dapatkan informasi lengkap spesifikasi, fitur, warna, dan harga ${car.name} terbaru. Kunjungi dealer Agung Toyota terdekat untuk test drive.`,
    keywords: `Harga ${car.name}, Spesifikasi ${car.name}, Kredit ${car.name}, Promo ${car.name}, Agung Toyota`,
  };
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex text-sm text-gray-500 dark:text-gray-400 mb-8" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-2">
            ...
            <li><span>/</span></li>
            <li className="text-gray-900 dark:text-white font-medium" aria-current="page">{car.name}</li>
          </ol>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-auto">
              <Link 
                href="/dealer" 
                className="flex-1 text-center px-6 py-4 bg-toyota-red hover:bg-toyota-darkred text-white font-bold rounded-xl shadow-lg transition-all hover:-translate-y-1"
              >
                Minta Penawaran
              </Link>
              <Link 
                href={`https://wa.me/6281380053428?text=Halo%20Agung%20Toyota,%20saya%20tertarik%20dengan%20${car.name}`} 
                target="_blank"
                className="flex-1 text-center px-6 py-4 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold rounded-xl shadow-lg transition-all hover:-translate-y-1"
              >
                Chat WhatsApp
              </Link>
            </div>
